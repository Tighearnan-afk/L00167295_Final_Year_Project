...
for testing_Image in os.listdir("Testing_images"):
    if path.isfile(f"Noisy_testing_images/{testing_Image}.jpg"):
        print(f"Testing_images/{testing_Image}.jpg already exists")
        continue
    ...
    image.save(f"Noisy_testing_images/{testing_Image}.jpg",quality=90)
...
for training_image in os.listdir("Training_images"):
    if path.isfile(f"Noisy_training_images/{training_image}.jpg"):
        print(f"Testing_images/{training_image}.jpg already exists")
        continue
    ...
    image = ski.util.img_as_ubyte(image)
    image = Image.fromarray(image).convert("L")
    image.save(f"Noisy_training_images/{training_image}.jpg",quality=90)
